...
public class HowDidYouDoItApplication {
    ...
    CommandLineRunner initDatabase(UserRepository userRepository, ProjectRepository projectRepository) {
        return args -> {
            ...
            // Busca o projeto original para fazer a reinterpretação
            Optional<Project> projetoOriginal = projectRepository.findById(2L); // Busca o projeto da Horta do Beto
            Long origemId = projetoOriginal.map(Project::getId).orElse(null);

            Project p4 = new Project(null, user1, "Horta Vertical com Irrigação Automática", "Adaptando o projeto do Beto com um toque de tecnologia usando Arduino.", 78, origemId, "## A Inspiração\n\nAdorei o projeto de **Horta Vertical com Garrafas PET** do @beto.construtor...");
